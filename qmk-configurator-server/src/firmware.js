...
const columnMultiplexMatrix = require('./columnMultiplexMatrix');
...
module.exports.setupFirmware = ({config, rules, configKeymap, keymap, indicators, staticIndicators}, callback) => {
  ...
  fs.mkdir(`/app/qmk_firmware/keyboards/${fd}/keymaps/default`, {recursive: true}, (err) => {
    if (err) throw err;
    fs.writeFile(`${firmwareLocation}/${fd}/config.h`, buildConfig(config), done);
    ...
    if (rules.multiplexColumns) {
      fs.writeFile(`${firmwareLocation}/${fd}/matrix.c`, columnMultiplexMatrix(), done);
    }
